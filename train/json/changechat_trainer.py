...
class ChangeChatTrainer(Trainer):
    ...
    def _save_checkpoint(self, model, trial=None, metrics=None):
        if getattr(self.args, "tune_mm_mlp_adapter", False):
            ...
            super(ChangeChatTrainer, self)._save_checkpoint(model, trial=trial)
...
model_dir = "./output"  # dossier où le modèle a été sauvegardé
...
# Charger le tokenizer, le modèle et le processor
...
for sample in data[:5]:
    ...
    im1_path = f"{image_folder}/im1/{img_id}"
    im2_path = f"{image_folder}/im2/{img_id}"

    # Charger et prétraiter les images
    image1 = Image.open(im1_path).convert("RGB")
    image2 = Image.open(im2_path).convert("RGB")
    # On empile les deux images pour simuler l'entrée bitemporelle
    images = [image1, image2]
    pixel_values = [processor(img, return_tensors="pt")["pixel_values"] for img in images]
    pixel_values = torch.cat(pixel_values, dim=0).unsqueeze(0)  # (1, 2, C, H, W)

    # Préparer l'entrée texte
    question = sample["question"]
    inputs = tokenizer(question, return_tensors="pt")

    # Adapter selon l'API de votre modèle (ici, on suppose un modèle standard)
    with torch.no_grad():
        output = model.generate(
            input_ids=inputs["input_ids"],
            # Si votre modèle accepte des images, ajoutez l'argument approprié ici
            # e.g. images=pixel_values
            max_new_tokens=50
        )
